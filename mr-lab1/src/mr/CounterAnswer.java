...
public class CounterAnswer extends Configured implements Tool
{
    public static void main(String[] args) throws Exception
    {
        int res = ToolRunner.run(new Configuration(), new CounterAnswer(), args);
        System.exit(res);
    }
    ...
    public int run(String[] args) throws Exception
    {
        ...
        Job job = new Job(getConf(), "CounterAnswer");
        job.setJarByClass(CounterAnswer.class);
...
//                System.out.println (Arrays.toString(tokens));
                ...
                for (String s : words)
                {
                    ...
                    {
                        numHashTags ++;
                        // increment counter, if we have more than one hashtag
                        if (numHashTags > 1)
                         {
                            context.getCounter(Counters.MORE_HASHTAGS).increment(1);
                        }
